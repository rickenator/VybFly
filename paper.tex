% PDF 1.4 with a classic cross-reference table: see docs/DATASET.md
\pdfminorversion=4
...
structural statistics, its propagation dynamics, its behavioral capabilities and its energy
...
benchmark on the real mushroom body acquires odor--reward associations (AUC $0.51\to1.00$ in 58
...
$\approx1.4\times10^{-4}$), weighted (a typical connection carries 12.6 synapses), organized into
...
program for complex networks~\cite{geom-renorm}: real networks embedded in a hidden metric
space can be renormalized by rescaling distances, and networks with scale-free degree
distributions show geometric scaling under that transformation. The program suggests a
...
Figure~\ref{fig:concept} lays out the object of study: one biological graph at the center, a
...
distributions), a triad-census divergence, the spectral distance between normalized Laplacian
spectra, a rich-club distance, a connectivity-matrix divergence over cell classes and neuropils,
community agreement (adjusted Rand index and normalized mutual information between Leiden
...
--- nearest-neighbor agglomeration with distances evaluated in the ambient Poincaré coordinates
...
perturbation scale tied to the local nearest-neighbor distance; each child inherits the parent's
...
\emph{neighboring} four-byte slot. A computed value with a zero high word therefore zeroed its
neighbor's cell --- producing exactly the \code{1818 0 0 0 0 0 0 0} pattern we observed. We filed
...
(2-D and 3-D, raw and axis-standardized), a 2-D hyperbolic (Poincaré) embedding learned by
stochastic gradient descent on the connection law, and normalized-Laplacian spectral embeddings
...
initialized without degree information --- so the direction of the published geometry result
...
it matters for behavior: community agreement falls to ARI 0.33/0.20/0.24 versus
...
\caption{Coarse-graining under both geometries. Lower is better for the left and center panels;
...
$R^2 = 0.999996$ --- the aggregate is emphatically not drifting toward an all-to-all regime.
...
most two steps --- the replicas reproduce the reference's propagation behavior, not merely its
degree sequence.}
\label{tab:dynamics}
...
six. This is the strongest evidence we have that coarse-graining preserves \emph{behavior} and
not just topology.
...
a light-odor protocol the network produces 366{,}872 spikes (2.64~Hz mean rate over 14.5\% of
...
Center: effective dimensionality grows sublinearly. Right: associative capacity saturates at the
...
teaching signal's sign drives the punished odor to AUC 0.000, symmetric with the rewarded one.
...
dopamine gate, and replacing the glomerular odor code with a degree-matched random projection.
...
conservation} (per-KC row normalization makes the summed readout invariant), and the punishment
...
returned a stale idle value at 96\% utilization, whereas NVML from the same thread tracked
...
\caption{Energy across scales. Left: measured GPU joules grow sublinearly. Center: cost per
spike falls by a factor of two --- a device-utilization effect, not biological efficiency.
...
task, while the dopamine-gating anatomy does. If that holds under better odor codes and multiple
...
the source's propagation behavior under identical normalized stimuli (active-set Jaccard
...
per spike for device-utilization reasons that must not be mistaken for biological efficiency,
...
gray matter of the brain. \textit{J. Cereb. Blood Flow Metab.} \textbf{21}, 1133--1145 (2001).
